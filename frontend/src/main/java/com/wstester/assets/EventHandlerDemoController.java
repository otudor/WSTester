...
public class EventHandlerDemoController{
	...
	private ObservableList<Table> tableViewDataXml = FXCollections.observableArrayList();
	private ObservableList<Table> tableViewDataXsd = FXCollections.observableArrayList();
	private ObservableList<Table> tableViewDataJson = FXCollections.observableArrayList();
	private ObservableList<Table> tableViewDataCsv = FXCollections.observableArrayList();
	private ObservableList<Table> tableViewDataXls = FXCollections.observableArrayList();
			...
			public void handle(ActionEvent event) {
				if(listView.getSelectionModel().getSelectedItem() != null){
	                ...
	                if(file != null && newValue.toString() !=null){
	                	try {
							...
							if(newValue.toString().toLowerCase().equals(extension.toString().toLowerCase())){
			                    ...
			                    .owner(null)
			                    .title("Error")
			                    .message("Please select " + newValue.toString() + " file format")
			                    .showError();
							}
						} catch (IOException e) {
							e.printStackTrace();
					...
					try {
	                    ...
	                    .owner(null)
	                    .title("Error")
	                    .message("Please select an asset where you want to add data!")
	                    .showError();
					} catch (Exception e) {
						e.printStackTrace();
					}
		...
		public void handle(ActionEvent event) {
			if(tableView.getSelectionModel().getSelectedItem() != null){
				...
				try {
                    ...
                    .owner(null)
                    .title("Error")
                    .message("Please select a file and then perform delete actions!")
                    .showError();
				} catch (Exception e) {
					e.printStackTrace();
				}
