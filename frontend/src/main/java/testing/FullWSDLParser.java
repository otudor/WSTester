...
public class FullWSDLParser {
    ...
    public static void main(String[] args) {
        ...
        Definitions defs = parser.parse("http://www.webservicex.net/geoipservice.asmx?WSDL");
 
        out("-------------- WSDL Details --------------");
        out("TargenNamespace: \t" + defs.getTargetNamespace());
        out("Style: \t\t\t" + defs.getLocalBindings());
        ...
        for (Message msg : defs.getMessages()) {
            ...
            for (Part part : msg.getParts()) {
                ...
                out("    Part Type: " + ((part.getType() != null) ? part.getType().getSchema() : "not available!" ));
                out("");
            }
        }
        out("");
